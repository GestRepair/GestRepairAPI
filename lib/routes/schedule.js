...
exports.list = function (req, res) {
  dbsql.mysql.getConnection(function (err, connection) {
    connection.query('SELECT s.idSchedule, v.registration as vehicle, se.nameService as service,s.date, s.isActive FROM `tbl_user_vehicle` uv, `tbl_vehicle` v, `tbl_schedule` s ,`tbl_service` se WHERE uv.vehicle = v.idVehicle and v.idVehicle = s.vehicle and s.service = se.idService and s.isActive =1 ;', function (error, results, fields) {
      connection.release();
      if (error) return res.status(500).json({ 'result': 'nok', 'message': 'Erro a listar agendamentos' ,'error': error });
      res.status(200).json({ 'result': 'ok','message':'Sucesso' ,'data': results });
    });
...
exports.listDayNow = function (req, res) {
  dbsql.mysql.getConnection(function (err, connection) {
    connection.query('SELECT sc.idSchedule, s.nameService as service, v.registration as vehicle, sc.date FROM tbl_schedule sc, tbl_service s, tbl_vehicle v where v.idVehicle = sc.vehicle and s.idService = sc.service and sc.date = CURDATE() and sc.isActive = 1 order by sc.date desc;', function (error, results, fields) {
      connection.release();
...
exports.listDayNext = function (req, res) {
  dbsql.mysql.getConnection(function (err, connection) {
    connection.query('SELECT sc.idSchedule, s.nameService as service, v.registration as vehicle, sc.date FROM tbl_schedule sc, tbl_service s, tbl_vehicle v where v.idVehicle = sc.vehicle and s.idService = sc.service and sc.date > CURDATE() and sc.isActive = 1 order by sc.date asc;', function (error, results, fields) {
      connection.release();
...
exports.listDayNowUser = function (req, res) {
  var id = req.params.id;
  dbsql.mysql.getConnection(function (err, connection) {
    connection.query('SELECT sc.idSchedule, s.nameService as service, v.registration as vehicle, sc.date FROM tbl_schedule sc, tbl_service s, tbl_vehicle v, tbl_user_vehicle uv where v.idVehicle = sc.vehicle and s.idService = sc.service and uv.vehicle = sc.vehicle and uv.user = ? and sc.date = CURDATE() and sc.isActive =1 order by sc.date desc;', id, function (error, results, fields) {
      connection.release();
      if (error) return res.status(500).json({ 'result': 'nok', 'message': 'Erro a encontrar as os agendamentos do dia de hoje' ,'error': error });
      res.status(200).json({ 'result': 'ok', 'message': 'Sucesso', 'data': results });
    });
...
exports.listDayNextUser = function (req, res) {
  var id = req.params.id;
  dbsql.mysql.getConnection(function (err, connection) {
    connection.query('SELECT sc.idSchedule, s.nameService as service, v.registration as vehicle, sc.date FROM tbl_schedule sc, tbl_service s, tbl_vehicle v, tbl_user_vehicle uv where v.idVehicle = sc.vehicle and s.idService = sc.service and uv.vehicle = sc.vehicle and uv.user = ? and sc.date > CURDATE() and sc.isActive = 1 order by sc.date asc;', id, function (error, results, fields) {
      connection.release();
      if (error) return res.status(500).json({ 'result': 'nok', 'message': 'Erro a encontrar as os agendamentos do dia seguinte' ,'error': error  });
      res.status(200).json({ 'result': 'ok', 'message': 'Sucesso', 'data': results });
    });
