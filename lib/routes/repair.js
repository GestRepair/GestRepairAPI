var dbsql = require('../../dbsql'),
  ...
  exports.info = function(req, res) {
    ...
    dbsql.mysql.getConnection(function(err, connection) {
      ...
      connection.query('SELECT r.idRepair,v.registration, r.description, r.price, sr.nameState, r.startDate, r.finishDate, r.information, em.employer, pr.part as part FROM tbl_part_repair pr,tbl_repair r, tbl_vehicle v, tbl_state_repair sr,tbl_user_vehicle uv,(SELECT er.repair,u.`name` as employer FROM tbl_employer_repair er,tbl_user u,tbl_employer e where e.idEmployer = er.employer and e.`user`=u.idUser) em where em.`repair` =r.idRepair  and r.vehicle = v.idVehicle and uv.vehicle=r.vehicle and r.state = sr.idstate and pr.`repair`=r.idRepair and uv.`user` = ? and r.idRepair = ?;', [id,repair], function(error, results, fields) {
            if (error)
            {
...
exports.listUser = function(req, res) {
  ...
  dbsql.mysql.getConnection(function(err, connection) {
    ...
    connection.query('SELECT r.idRepair,v.registration, r.description, r.price, sr.nameState, r.startDate, r.finishDate, r.information, em.employer FROM tbl_repair r, tbl_vehicle v, tbl_state_repair sr,tbl_user_vehicle uv,(SELECT er.repair,u.`name` as employer FROM tbl_employer_repair er,tbl_user u,tbl_employer e where e.idEmployer = er.employer and e.`user`=u.idUser) em where em.`repair` =r.idRepair  and r.vehicle = v.idVehicle and uv.vehicle=r.vehicle and r.state = sr.idstate and uv.`user` = ?', id, function(error, results, fields) {
      connection.release();
