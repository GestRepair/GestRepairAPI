...
exports.listtype = function(req, res) {
  dbsql.mysql.getConnection(function(err, connection) {
    ...
    connection.query('SELECT ut.idUser,ut.username,ut.name,ut.street,ut.zipcode,ut.city,ut.email,ut.nif,ut.contact,ut.isActive FROM tbl_user ut where ut.role = ?',req.params.id ,function(error, results, fields) {
      connection.release();
...
exports.addEmployer = function(req, res) {
  ...
  dbsql.mysql.getConnection(function(err, connection) {
    ...
    connection.query('INSERT INTO tbl_employer SET ?', valuesEmployer, function(error, results, fields) {
      ...
      connection.query('UPDATE tbl_user SET role = ? where idUser = ?', [role,user], function(error, results, fields) {
        if (error) {
          connection.release();
          return res.status(500).json({ 'result': 'nok', 'message': error });
        }
