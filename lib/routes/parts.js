...
exports.listPartNotService = function (req, res) {
    var id = req.params.id;
    dbsql.mysql.getConnection(function (err, connection) {
        connection.query('SELECT s.idService, s.nameService FROM (SELECT s.idService FROM (SELECT ps.part, ps.service FROM tbl_part_service ps) pps, tbl_service s where pps.service = idService and pps.part = ? order by s.idService) ppps, tbl_service s where s.idService <> ppps.idService and s.idService <> 1 and s.idService <> 2  ;', id, function (error, results, fields) {
            connection.release();
            if (error) return res.status(500).json({ 'result': 'nok', 'message': error });
            res.status(200).json({ 'result': 'ok', 'message': 'Sucess', 'data': results });
        });
