...
exports.listDays = function (req, res) {
  dbsql.mysql.getConnection(function (err, connection) {
    connection.query('select * from tbl_schedule_date WHERE (CASE WHEN idDate = 1 THEN (select EXISTS(SELECT * FROM tbl_schedule WHERE date = CURDATE())) WHEN idDate = 2 THEN (select EXISTS(SELECT * FROM tbl_schedule WHERE date < CURDATE())) WHEN idDate = 3 THEN (select EXISTS(SELECT * FROM tbl_schedule WHERE date > CURDATE())) END);', function (error, results, fields) {
      connection.release();
