...
exports.listServiceEmployer = function (req, res) {
  dbsql.mysql.getConnection(function (err, connection) {
    connection.query('SELECT s.idService, s.nameService FROM tbl_employer e, tbl_service s where s.idService = e.service and e.isActive = ? group by s.idService;',req.params.id, function (error, results, fields) {
      connection.release();
      if (error) return res.status(500).json({ 'result': 'nok', 'message': 'Erro a listar funcionários por serviço', 'error': error });
      res.status(200).json({ 'result': 'ok', 'data': results,'message':'Sucesso' });
    });
