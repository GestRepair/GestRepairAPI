  ...
  dbsql.mysql.getConnection(function (err, connection) {
    async.waterfall([
      ...
      function (error, callback) {
        connection.query('SELECT amount FROM tbl_part WHERE idPart = ?', parseInt(req.params.id), function (error, results) {
          callback(null, error, results);
        });
      },
      function (error, results, callback) {
        var am = parseInt(results[0].amount) - 1;
        connection.query('UPDATE tbl_part SET amount = ? WHERE idPart = ?', [am, parseInt(req.params.id)], function (error, results) {
          callback(error);
        });
      }
