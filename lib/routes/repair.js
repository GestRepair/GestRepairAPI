...
exports.listUser = function (req, res) {
  var id = req.params.id;
  dbsql.mysql.getConnection(function (err, connection) {
    connection.query('SELECT r.idRepair, v.registration, r.description, r.price, sr.nameState, r.startDate, r.finishDate, r.information FROM tbl_repair r, tbl_user_vehicle uv, tbl_vehicle v, tbl_state_repair sr, tbl_employer_repair er, tbl_part_repair pr where uv.vehicle = r.vehicle and v.idVehicle = r.vehicle and sr.idstate = r.state and er.repair = r.idRepair and pr.repair = r.idRepair and uv.user = ?  group by r.idRepair order by r.startDate desc;', id, function (error, results, fields) {
      connection.release();
      if (error) {
