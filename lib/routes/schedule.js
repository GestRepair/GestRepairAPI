var dbsql = require('../../dbsql'),
  ...
  exports.info = function(req, res) {
    ...
    dbsql.mysql.getConnection(function(err, connection) {
      ...
      connection.query('SELECT s.idSchedule, v.registration as vehicle, se.nameService as service,s.date FROM `tbl_utilizadores_veiculos` uv, `tbl_vehicle` v, `tbl_schedule` s ,`tbl_service` se WHERE uv.numVei = v.idVehicle and v.idVehicle = s.vehicle and s.service = se.idService and s.isActive = 1 and uv.numUser = ? and s.idSchedule = ?;', [id,schedule], function(error, results, fields) {
        connection.release();
  ...
  exports.listUser = function(req, res) {
    ...
    dbsql.mysql.getConnection(function(err, connection) {
      ...
      connection.query('SELECT s.idSchedule, v.registration as vehicle, se.nameService as service,s.date FROM `tbl_utilizadores_veiculos` uv, `tbl_vehicle` v, `tbl_schedule` s ,`tbl_service` se WHERE uv.numVei = v.idVehicle and v.idVehicle = s.vehicle and s.service = se.idService and s.isActive = 1 and uv.numUser = ?;',id, function(error, results, fields) {
        connection.release();
